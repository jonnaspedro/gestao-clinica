...
# 1) Níveis de especialização médica (não podem ser alterados) [cite: 1]
# Uma tupla é usada para garantir que os valores não possam ser alterados.
...
# 2) Banco de Dados de Procedimentos Médicos [cite: 2, 3]
# Uma lista de dicionários para armazenar os procedimentos.
procedimentos_db = [
...
# 3) e 4) Estrutura do Banco de Dados de Médicos e Médico Exemplo [cite: 4, 5, 6]
# Uma lista de dicionários para o banco de dados de médicos.
medicos_db = []

# Exemplo de dados de um médico a ser armazenado [cite: 5]
medico_exemplo = {
    "matricula": "12345",
    "cpf": "111.222.333-44",
    "nome": "Dr. João Silva",
...
# Armazena o médico de exemplo no banco de dados [cite: 6]
medicos_db.append(medico_exemplo)
print("\n--- Médico de Exemplo Adicionado ao Banco de Médicos ---")
for medico in medicos_db:
    print(medico)

# 5) Função para Adicionar Médico ao Banco de Dados [cite: 7]
def adicionar_medico(novo_medico, banco_medicos):
    """
    Adiciona um novo médico ao banco de dados de médicos. [cite: 7]
    Args:
...
# --- Funções Auxiliares para Manipulação do Banco de Médicos (Questão 4) ---
# Adições para "adicionar, remover e alterar" como mencionado na questão 4 [cite: 5]
...
# 6) Função para Calcular Salário-Base do Médico [cite: 8]
...
# 7) Função para Imprimir Dados do Médico [cite: 9, 10]
def imprimir_dados_medico(matricula, banco_medicos, banco_procedimentos):
    """
    Imprime os dados completos de um médico no formato especificado. [cite: 9, 10]
    Args:
...
# Adicionando o "Seu Zé" conforme o exemplo da prova para testar impressão [cite: 12]
medico_seu_ze = {
    "matricula": "11111",
    "cpf": "777.888.999-00",
    "nome": "Seu Zé",
